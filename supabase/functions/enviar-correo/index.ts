...
const corsHeaders = {
    'Access-Control-Allow-Origin': 'https://www.trotapie.com',
...
const GOOGLE_SCRIPT_EXEC_URL =
    'https://script.google.com/macros/s/AKfycbxojdZ6fXbfu7ji0mIGGaMB1JO9SMIs0kqJW6NsWbyToFcXHuSKGPDKkU5598zTRU7q/exec';
...
const GIF_FIRMA_URL =
    'https://drive.google.com/uc?export=view&id=1re5mlzByXy_qZHV0bRVRsJ29W3cZHKim';
...
Deno.serve(async (req) => {
    ...
    try {
        console.log(
            `${LOG_PREFIX}[${requestId}] Inicio request. method=${req.method}`
        );

        if (req.method !== 'POST') {
            console.warn(
                `${LOG_PREFIX}[${requestId}] Metodo no permitido: ${req.method}`
            );
            return new Response(
                JSON.stringify({
                    ok: false,
        ...
        if (!correos.length) {
            console.warn(
                `${LOG_PREFIX}[${requestId}] Request sin correo destino.`
            );
            return new Response(
                JSON.stringify({
                    ok: false,
        ...
        const correosInvalidos = correos.filter(
            (email) => !isValidEmail(email)
        );
        if (correosInvalidos.length) {
            console.warn(
                `${LOG_PREFIX}[${requestId}] Correos invalidos: ${correosInvalidos.join(', ')}`
            );
            return new Response(
                JSON.stringify({
                    ok: false,
...
${cotizacionUrl ? `Consulta tu cotizacion: ${cotizacionUrl}` : ''}
...
Te atendio: ${firma?.nombre || remitenteNombre}.
    `.trim();
        ...
        if (googleScriptResult.failedEmails.length === 0) {
            console.log(
                `${LOG_PREFIX}[${requestId}] Envio completado por Google Script.`
            );
            return new Response(
                JSON.stringify({
                    ok: true,
        ...
        if (!mailerooApiKey) {
            console.error(
                `${LOG_PREFIX}[${requestId}] Sin MAILEROO_API_KEY para fallback.`
            );
            return new Response(
                JSON.stringify({
                    ok: false,
                    message:
                        'Google Script no pudo enviar todos los correos y no existe MAILEROO_API_KEY para respaldo.',
                    googleScriptResult,
                }),
                {
                    status: 502,
        ...
        const destinatariosFallback =
            googleScriptResult.failedEmails.length > 0
                ? googleScriptResult.failedEmails
                : correos;
        console.log(
        ...
        const mailPayload: Record<string, unknown> = {
            ...
            to: destinatariosFallback.map((address) => ({
                address,
        ...
        console.log(
            `${LOG_PREFIX}[${requestId}] Envio completado por Maileroo fallback.`
        );
        return new Response(
            JSON.stringify({
                ok: true,
...
async function obtenerFirmaEmpleado(
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        console.warn(
            `${LOG_PREFIX} No se pudo cargar la firma: faltan credenciales de Supabase.`
        );
        return null;
    }
    ...
    if (solicitudError || !solicitud?.empleado_id) {
        if (solicitudError)
            console.warn(
                `${LOG_PREFIX} No se pudo localizar la solicitud para la firma.`
            );
        return null;
    }
    ...
    if (empleadoError || !empleado) {
        if (empleadoError)
            console.warn(
                `${LOG_PREFIX} No se pudieron cargar los datos del empleado.`
            );
        return null;
    }
    ...
    return {
        nombre:
            String(empleado.nombre ?? '').trim() || DEFAULT_REMITENTE_NOMBRE,
        cargo: String((empleado as any)?.cargo?.rol ?? '').trim(),
...
function construirFirmaHtml(firma: FirmaEmpleado | null): string {
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="width:100%;border-collapse:collapse;font-size:0;line-height:0;">
            <tr>
              ...
              <td width="24%" align="center" valign="middle" style="padding:0;background:#ffffff;font-size:0;line-height:0;">
                <a href="${APP_URL}" style="text-decoration:none;"><img src="${GIF_FIRMA_URL}" alt="Trotapie" width="300" style="display:block;width:100%;max-width:300px;height:auto;margin:0 auto;border:0;"></a>
              </td>
    ...
    <map name="firma-contacto" id="firma-contacto">
      <area shape="rect" coords="35,285,460,345" href="mailto:jtriego@trotapie.com" alt="Enviar correo a Javier Triego">
      <area shape="rect" coords="35,365,390,440" href="https://wa.me/526691957651" alt="Enviar WhatsApp a Javier Triego">
    </map>
    <map name="firma-redes" id="firma-redes">
      ...
      <area shape="rect" coords="565,0,700,165" href="https://wa.me/526691957651" alt="WhatsApp de Javier Triego">
      <area shape="rect" coords="710,0,850,165" href="${APP_URL}" alt="Sitio web de Trotapie">
    </map>`;
}

function construirCtaCotizacionHtml(cotizacionUrl: string): string {
    if (!cotizacionUrl) return '';

    const url = escapeHtml(cotizacionUrl);
    return `
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0;border-collapse:separate;">
      <tr>
        <td align="center" bgcolor="#008f7a" style="border-radius:10px;background:#008f7a;">
          <a href="${url}" style="display:inline-block;padding:14px 22px;border:1px solid #008f7a;border-radius:10px;color:#ffffff;font-family:Arial,sans-serif;font-size:15px;font-weight:700;line-height:20px;text-align:center;text-decoration:none;">Consultar mi cotizacion</a>
        </td>
      </tr>
    </table>
    <p style="margin:0 0 4px;color:#4b5563;font-family:Arial,sans-serif;font-size:12px;line-height:18px;">Tambien puedes copiar este enlace en tu navegador:</p>
    <p style="margin:0;color:#008f7a;font-family:Arial,sans-serif;font-size:12px;line-height:18px;word-break:break-all;"><a href="${url}" style="color:#008f7a;text-decoration:underline;">${url}</a></p>
  `;
}
...
async function enviarPrimeroConGoogleScript(
    input: GoogleScriptSendInput
): Promise<GoogleScriptSendResult> {
    ...
    for (let i = 0; i < input.correos.length; i += 1) {
        ...
        console.log(
            `${LOG_PREFIX}[${input.requestId}] Intentando Google Script para ${correo}.`
        );
        try {
                ...
                for (const correoRestante of restantes) {
                    ...
                    results.push({
                        ...
                        message: data?.message
                            ? String(data.message)
                            : 'Google Script no puede enviar mas correos por ahora.',
                        data,
                    });
                }
                break;
