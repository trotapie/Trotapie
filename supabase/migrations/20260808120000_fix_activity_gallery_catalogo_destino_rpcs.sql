-- Las pantallas administrativas ya envian IDs de catalogo_destinos.
-- Estas RPC conservan p_destino_id como contrato publico, pero lo resuelven
-- contra detalles_destinos.catalogo_destino_id.
...
begin
  select id
   ...
   where catalogo_destino_id = p_destino_id
   limit 1;
  ...
  if p_accion = 'delete_empty' then
    ...
    return jsonb_build_object('ok', true, 'accion', p_accion, 'carpeta_id', p_carpeta_id);
  end if;
  ...
  if p_accion = 'create_and_move' then
    ...
    if not found then
      ...
      returning id, atraccion_id, nombre, orden into v_carpeta_destino;
    end if;
...
begin
  ...
  delete from public.atracciones_imagenes where atraccion_id = p_actividad_id;
  get diagnostics v_imagenes_eliminadas = row_count;

  delete from public.atracciones_carpetas where atraccion_id = p_actividad_id;
