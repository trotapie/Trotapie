...
@Component({
  ...
  template: `
    <div class="switch-row">
      ...
      <button type="button" class="switch" [class.switch--checked]="value" [disabled]="disabled" [attr.aria-label]="label" [attr.aria-checked]="value" role="switch" (click)="update(!value)">
        <div class="slider">
          <div class="circle">
  ...
  styles: [`
    ...
    .switch-label { font-size: 14px; color: #374151; }
     .switch { --switch-width: 46px; --switch-height: 24px; --switch-bg: rgb(131, 131, 131); --switch-checked-bg: rgb(0, 218, 80); --circle-diameter: 18px; --switch-offset: calc((var(--switch-height) - var(--circle-diameter)) / 2); --switch-transition: all .2s cubic-bezier(.27,.2,.25,1.51); --circle-bg: #fff; --circle-shadow: 1px 1px 2px rgba(146,146,146,.45); --circle-checked-shadow: -1px 1px 2px rgba(163,163,163,.45); display: inline-block; width: var(--switch-width); height: var(--switch-height); min-width: 0; min-height: 0; border: 0; padding: 0; background: transparent; line-height: 0; appearance: none; cursor: pointer; }
     .switch:hover, .switch:active { transform: none; background: transparent; box-shadow: none; }
    ...
    .slider::before { content: ''; position: absolute; width: calc(var(--circle-diameter) / 2); height: calc(var(--circle-diameter) / 4 - 1px); left: calc(var(--switch-offset) + var(--circle-diameter) / 4); background: var(--circle-bg); border-radius: 1px; transition: all .2s ease-in-out; }
     .switch--checked .slider { background: var(--switch-checked-bg); }
     .switch--checked .slider .checkmark { transform: scale(1); }
     .switch--checked .slider .cross { transform: scale(0); }
     .switch--checked .slider::before { left: calc(100% - var(--circle-diameter) / 2 - var(--circle-diameter) / 4 - var(--switch-offset)); }
     .switch--checked .slider .circle { left: calc(100% - var(--circle-diameter) - var(--switch-offset)); box-shadow: var(--circle-checked-shadow); }
     .switch:focus-visible .slider { outline: 2px solid #2563eb; outline-offset: 2px; }
     .switch:disabled .slider { cursor: not-allowed; opacity: .55; }
  `],
